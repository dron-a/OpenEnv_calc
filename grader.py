...
def compute_score(state: AdPlatformState) -> float:
    """Grader for task1: budget allocation."""
...
def compute_auction_score(state: AdPlatformState) -> dict:
    """
    Independent evaluation of auction task performance.
    Returns a breakdown dict and a final score in [0, 1].

    Scoring dimensions:
      - conversion_score  (0.6): total_conversions / theoretical maximum
...
def compute_multi_campaign_score(state: AdPlatformState) -> dict:
    """
    Independent evaluation of multi_campaign task performance.
    Returns a breakdown dict and a final score in [0, 1].
